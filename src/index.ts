...
(async () => {
	...
	if (!response.token || !response.guildId || !response.channelId || !response.fileType) return process.exit(1);

	new Scraper(
		response.token,
		response.guildId,
		response.channelId,
		response.fileType as FileType,
		response.afterMessageId,
	);
})();
