	...
	public async addMessage(m: any): Promise<void> {
		if (this.fileType === FileType.Html) {
			...
			const avatar = m.author.avatar
				? `https://cdn.discordapp.com/avatars/${m.author.id}/${m.author.avatar}.webp`
		...
		if (m.sticker_items && m.sticker_items.length !== 0)
			m.content += ` ${m.sticker_items
				.map((s) => `https://media.discordapp.net/stickers/${s.id}.webp?size=160`)
				.join(' ')};`;
		if (m.attachments && m.attachments.length !== 0) m.content += ` ${m.attachments.map((a) => a.url).join(' ')}`;

		await fs
			.appendFile(
				`output/channel_${this.channelId}/channel.txt`,
				`[${m.id} | ${timestampToTime(m.timestamp)}] (${m.id}) ${m.author.username}#${m.author.discriminator} -> ${
					m.content
				}  \n`,
			)
			.catch((e) => e);
